...
async function main() {
  ...
  const dueDate = new Date(currentYear, currentMonth, 10); // Due on 10th of next month
  ...
  // Fee for student2 (partially paid)
  const fee2 = await prisma.fee.create({
    data: {
      ...
      fine: 0,
      previous_due: 0,
      discount: 500, // Scholarship discount
      total_amount: 9500,
      paid_amount: 5000,
      balance_amount: 4500,
